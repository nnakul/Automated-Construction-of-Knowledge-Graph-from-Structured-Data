...
// explicitly labelled must be obeyed
...
class LinguisticMatching {
    ...
    public:
        ...
        static double FreqVectorL2Norm ( const string & s1, const string & s2 ) {
            vec<double> f1(26, 0), f2(26, 0);
            for ( char c : s1 ) {
                char l = tolower(c);
                int i = c - 'a';
                if ( i < 26 )   f1[i]+=1.;
...
class RandomForestActiveLearner {
    private:
        ...
        void Initialize ( ) {
            ...
            bool label = LabelManually(firstEnt(it->second), secEnt(it->second));
            labelled.push_back({featVec(it->second), label});
            if ( label )
        ...
        void LabelNextK ( int k ) {
            ...
            while ( ! topk.empty() ) {
                ...
                bool label = LabelManually(e1, e2);
        ...
        bool ActiveLearning ( ) {
            ...
            // cout<<train_len<<' '<<labelled.size()<<endl;
...
class RecordLinker {
    private:
        ...
        static vec<double> PairFeatureVector ( const string & e1 , const string & e2 ) {
            return {
                1. * LinguisticMatching::EditDistance(e1, e2) / min<int>(e1.size(), e2.size()),
